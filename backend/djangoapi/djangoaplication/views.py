...
from django.http import HttpResponse, JsonResponse  # Importa HttpResponse y JsonResponse
...
def subir_video(request):
  if request.method == 'POST':
    try:
      ...
      if not video_file:
        return JsonResponse({'error': 'No se proporcionó un archivo de video.'}, status=400)
      ...
      return JsonResponse({'mensaje': 'Video subido correctamente.', 'idVideo': video.idVideo}, status=201)
...
def subir_audio(request):
  if request.method == 'POST':
    try:
      ...
      return JsonResponse({'mensaje': 'Audio subido correctamente.', 'idAudio': audio.idAudio}, status=201)
    except Exception as e:
      return JsonResponse({'error': str(e)}, status=500)
  else:
    return JsonResponse({'error': 'Método no permitido. Use POST.'}, status=405)